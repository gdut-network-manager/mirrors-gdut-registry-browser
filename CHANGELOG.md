...
* 层信息进度条暗色模式下可见度提升
  - 亮色模式: 0.10→0.25, hover 0.18→0.35
  - 暗色模式: 新增 .dark-theme 专用 0.4, hover 0.5

# v2.5.16

* 层信息进度条改为行背景, 各层进度条首尾相接
  - 每行 tr 用 --bar-start/--bar-end CSS 变量控制渐变区间
  - 第一层从 0% 开始, 最后一层到 100% 结束, 中间各层首尾衔接
  - hover 时加深背景透明度 (0.10→0.18)

# v2.5.15

* 层信息部分改为表格样式, 与其他 section 保持一致
  - p 标签 + 共享进度条 → table(#/Digest/大小/占比)
  - 每层独立进度条 + 百分比文字, 修复除第一层外进度条不显示的 bug
  - 根因: 旧逻辑每行渲染所有层的进度条段, 仅当前层着色其余 transparent, 显示异常
...
* 首页 >2K (≥2560px) 左右各留 1/12 视口间隔, 不再贴边
* 首页 <2K 保持撑满全宽, 移动端不受影响
